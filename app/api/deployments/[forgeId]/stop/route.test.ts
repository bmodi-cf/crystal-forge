...
describe('deployment stop route dev guard', () => {
  it('returns 404 in dev mode before doing any work', async () => {
    process.env.FORGE_DASHBOARD_MODE = 'dev';
    const { POST } = await import('./route');
    const res = await POST(
      new Request('http://localhost/api/deployments/x/stop', { method: 'POST' }) as unknown as NextRequest,
      { params: Promise.resolve({ forgeId: 'x' }) } as unknown as RouteContext<'/api/deployments/[forgeId]/stop'>,
    );
